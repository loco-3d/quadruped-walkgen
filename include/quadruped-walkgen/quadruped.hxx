...
void ActionModelQuadrupedTpl<Scalar>::calc(const boost::shared_ptr<crocoddyl::ActionDataAbstractTpl<Scalar> >& data,
  ...
  if (implicit_integration) {
    d->xnext.template head<6>() =
        d->xnext.template head<6>() + A.topRightCorner(6, 6).diagonal().cwiseProduct(d->xnext.tail(6));
  } else {
    d->xnext.template head<6>() =
...
template <typename Scalar>
void ActionModelQuadrupedTpl<Scalar>::set_max_fz_contact(const Scalar& max_fz_) {
  // The model need to be updated after this changed
  max_fz = max_fz_;
  for (int i = 0; i < 4; i = i + 1) {
    ub(6 * i + 5) = max_fz;
