...
template <typename Scalar>
void ActionModelQuadrupedStepPeriodTpl<Scalar>::calc(const boost::shared_ptr<crocoddyl::ActionDataAbstractTpl<Scalar> >& data,
  ...
  rub_max_ << dt_min_ - x.tail(1) , x.tail(1) - dt_max_ , u[0]*u[0] + u[1]*u[1] - beta_lim*x[20]*x[20] , u[2]*u[2] + u[3]*u[3] - beta_lim*x[20]*x[20]  ; 
  
  rub_max_bool = (rub_max_.array() >= Scalar(0.)).matrix().template cast<Scalar>() ; 
  rub_max_ = rub_max_.cwiseMax(Scalar(0.)) ; 

  // d->cost = Scalar(0.5) * d->r.transpose() * d->r   + dt_bound_weight * Scalar(0.5) * rub_max_.head(2).squaredNorm() + speed_weight * Scalar(0.5) * rub_max_.tail(2).squaredNorm();
  d->cost = Scalar(0.5) * d->r.transpose() * d->r   + dt_bound_weight * Scalar(0.5) * rub_max_.head(2).squaredNorm() + speed_weight * Scalar(0.5) * rub_max_.tail(2).sum();
}


template <typename Scalar>
void ActionModelQuadrupedStepPeriodTpl<Scalar>::calcDiff(const boost::shared_ptr<crocoddyl::ActionDataAbstractTpl<Scalar> >& data,
                                              const Eigen::Ref<const typename MathBase::VectorXs>& x,
                                              const Eigen::Ref<const typename MathBase::VectorXs>& u) {
  if (static_cast<std::size_t>(x.size()) != state_->get_nx()) {
    throw_pretty("Invalid argument: "
  ...
  d->Lx.template tail<1>() <<  dt_bound_weight *(- rub_max_[0] + rub_max_[1]) - beta_lim*speed_weight*x(20)*rub_max_bool[2] - beta_lim*speed_weight*x(20)*rub_max_bool[3] ; 
  ...
  d->Lu << speed_weight*u[0]*rub_max_bool[2] , speed_weight*u[1]*rub_max_bool[2] , 
           speed_weight*u[2]*rub_max_bool[3],  speed_weight*u[3]*rub_max_bool[3] ,
           Scalar(0.);
  ...
  d->Lxx.diagonal().tail(1) << dt_weight_*dt_weight_ + dt_bound_weight * rub_max_bool[0] + dt_bound_weight *rub_max_bool[1]; 
  ...
  d->Luu.diagonal() << speed_weight*rub_max_bool[2] ,
                       speed_weight*rub_max_bool[2] ,
                       speed_weight*rub_max_bool[3] , 
                       speed_weight*rub_max_bool[3] ,
                       Scalar(0.)  ; 
  ...
  //                      dt_weight_*dt_weight_ - speed_weight*Scalar(2)*beta_lim*(rub_max_[2] - 2*beta_lim*u[4]*u[4])*rub_max_bool[2] 
  //                      -speed_weight*Scalar(2)*beta_lim*(rub_max_[3] - 2*beta_lim*u[4]*u[4])*rub_max_bool[3]  ; 
  // d->Luu.diagonal().head(4) += (step_weights_.array() * step_weights_.array()).matrix() ;
...
template <typename Scalar>
void ActionModelQuadrupedStepPeriodTpl<Scalar>::update_model(const Eigen::Ref<const typename MathBase::MatrixXs>& l_feet  ,
