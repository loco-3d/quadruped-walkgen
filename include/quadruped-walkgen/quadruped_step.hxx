...
ActionModelQuadrupedStepTpl<Scalar>::ActionModelQuadrupedStepTpl()
  ...
  position_.setZero();  // Xk+1 = Xk + Uk, Xk does not correspond to the current position of the flying feet, Delta_x is not straightforward
  ...
  beta_x_ = Eigen::Array<Scalar, Eigen::Dynamic, Eigen::Dynamic>::Zero(N_sampling - 1, 4);  // Depends on a0_x, v0_x of feet
  beta_y_ = Eigen::Array<Scalar, Eigen::Dynamic, Eigen::Dynamic>::Zero(N_sampling - 1, 4);  // Depends on a0_y, v0_y of feet
  ...
  beta_x_v = Eigen::Array<Scalar, Eigen::Dynamic, Eigen::Dynamic>::Zero(N_sampling - 1, 4);  // Depends on a0_x, v0_x of feet
  beta_y_v = Eigen::Array<Scalar, Eigen::Dynamic, Eigen::Dynamic>::Zero(N_sampling - 1, 4);  // Depends on a0_y, v0_y of feet
...
void ActionModelQuadrupedStepTpl<Scalar>::calc(
  ...
  if (is_acc_activated_){
    for (int i =0;i<4;i++){
      if (S_(i) == Scalar(1.)){
        ...
        // rb_accx_max_.col(2*i) = (x(12+ 2*i) + u(2*i) - position_(0,i))*alpha_ + beta_x_.col(i) - acc_lim_(0)*tmp_ones_;
        // rb_accx_max_.col(2*i+1) = -( x(12+ 2*i) + u(2*i) - position_(0,i) )*alpha_ - beta_x_.col(i) - acc_lim_(0)*tmp_ones_;

        // rb_accy_max_.col(2*i) = ( x(12+ 2*i+1) + u(2*i+1) - position_(1,i) )*alpha_ + beta_y_.col(i) - acc_lim_(1)*tmp_ones_;
        // rb_accy_max_.col(2*i+1) = -( x(12+ 2*i+1) + u(2*i+1) - position_(1,i) )*alpha_ - beta_y_.col(i) - acc_lim_(1)*tmp_ones_;

        // position_ expressed in world frame
        rb_accx_max_.col(2*i) = (oRh_(0,0)*(x(12+ 2*i) + u(2*i)) + oRh_(0,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(0) - position_(0,i))*alpha_ + beta_x_.col(i) - acc_lim_(0)*tmp_ones_;
        rb_accx_max_.col(2*i+1) = -(oRh_(0,0)*(x(12+ 2*i) + u(2*i)) + oRh_(0,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(0) - position_(0,i))*alpha_ + beta_x_.col(i) - acc_lim_(0)*tmp_ones_;;

        rb_accy_max_.col(2*i) = ( oRh_(1,0)*(x(12+ 2*i) + u(2*i)) + oRh_(1,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(1) - position_(1,i) )*alpha_ + beta_y_.col(i) - acc_lim_(1)*tmp_ones_;
        rb_accy_max_.col(2*i+1) = -( oRh_(1,0)*(x(12+ 2*i) + u(2*i)) + oRh_(1,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(1) - position_(1,i) )*alpha_ - beta_y_.col(i) - acc_lim_(1)*tmp_ones_;
      }
      else{
  ...
  if (is_vel_activated_){
    for (int i =0;i<4;i++){
      if (S_(i) == Scalar(1.)){
        ...
        // rb_velx_max_.col(2*i) = (x(12+ 2*i) + u(2*i) - position_(0,i))*alpha_v + beta_x_v.col(i) - vel_lim_(0)*tmp_ones_;
        // rb_velx_max_.col(2*i+1) = -( x(12+ 2*i) + u(2*i) - position_(0,i) )*alpha_v - beta_x_v.col(i) - vel_lim_(0)*tmp_ones_;

        // rb_vely_max_.col(2*i) = ( x(12+ 2*i+1) + u(2*i+1) - position_(1,i) )*alpha_v + beta_y_v.col(i) - vel_lim_(1)*tmp_ones_;
        // rb_vely_max_.col(2*i+1) = -( x(12+ 2*i+1) + u(2*i+1) - position_(1,i) )*alpha_v - beta_y_v.col(i) - vel_lim_(1)*tmp_ones_;

        // position_ expressed in world frame
        rb_velx_max_.col(2*i) = (oRh_(0,0)*(x(12+ 2*i) + u(2*i)) + oRh_(0,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(0) - position_(0,i))*alpha_v + beta_x_v.col(i) - vel_lim_(0)*tmp_ones_;
        rb_velx_max_.col(2*i+1) = -(oRh_(0,0)*(x(12+ 2*i) + u(2*i)) + oRh_(0,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(0) - position_(0,i))*alpha_v - beta_x_v.col(i) - vel_lim_(0)*tmp_ones_;

        rb_vely_max_.col(2*i) = ( oRh_(1,0)*(x(12+ 2*i) + u(2*i)) + oRh_(1,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(1) - position_(1,i) )*alpha_v + beta_y_v.col(i) - vel_lim_(1)*tmp_ones_;
        rb_vely_max_.col(2*i+1) = -( oRh_(1,0)*(x(12+ 2*i) + u(2*i)) + oRh_(1,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(1) - position_(1,i) )*alpha_v - beta_y_v.col(i) - vel_lim_(1)*tmp_ones_;
      }
      else{
  ...
  if (is_jerk_activated_){
    for (int i =0;i<4;i++){
      if (S_(i) == Scalar(1.)){
        rb_jerk_(0,i) = (oRh_(0,0)*(x(12+ 2*i) + u(2*i)) + oRh_(0,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(0) - position_(0,i) ) * alpha_j + beta_j(0,i) - jerk_(0,i);
        rb_jerk_(1,i) = (oRh_(1,0)*(x(12+ 2*i) + u(2*i)) + oRh_(1,1)*(x(12+ 2*i+1) + u(2*i+1)) + oTh_(1) - position_(1,i) ) * alpha_j + beta_j(1,i) - jerk_(1,i);
...
void ActionModelQuadrupedStepTpl<Scalar>::update_model(const Eigen::Ref<const typename MathBase::MatrixXs>& l_feet,
  ...
  for (int i=0; i<4; i++){
    if (S[i] == Scalar(1) && is_acc_activated_) {
      beta_x_.col(i) = acceleration(0,i)*tmp_ones_ + (velocity(0,i) / delta_T) * gamma_.col(1) + acceleration(0,i)*gamma_.col(2); 
      beta_y_.col(i) = acceleration(1,i)*tmp_ones_ + (velocity(1,i) / delta_T) * gamma_.col(1) + acceleration(1,i)*gamma_.col(2);       
    }
    else{
    ...
    if (S[i] == Scalar(1) && is_vel_activated_) {
      beta_x_v.col(i) = velocity(0,i)*tmp_ones_ + (acceleration(0,i) * delta_T) * gamma_v.col(1) + velocity(0,i)*gamma_.col(2) + (acceleration(0,i) * delta_T) * gamma_v.col(3); 
      beta_y_v.col(i) = velocity(1,i)*tmp_ones_ + (acceleration(1,i) * delta_T) * gamma_v.col(1) + velocity(1,i)*gamma_.col(2) + (acceleration(1,i) * delta_T) * gamma_v.col(3);
    }
    else{
